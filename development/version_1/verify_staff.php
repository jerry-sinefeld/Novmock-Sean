...
$enginid = $_SESSION["enginid"] ?? null; // if this value exists use it otherwise null
if (!$enginid) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["request_code"])) {
        $auth_code = code_request($conn, $enginid);
        if ($auth_code) {
    ...
    if (isset($_POST["submit_verification"])) {
        ...
        if (auth($conn, $submitted_code)) {//authenticates
            if (activate_engin($conn, $enginid)) {//sets engineer as active
