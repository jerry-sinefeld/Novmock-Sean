...
function staff_login ($conn, $post){
        ...
        $sql = "SELECT * FROM employees WHERE username = ?"; //select everything from the user table where username = the entered username
        $stmt = $conn->prepare($sql);//prepare the sql for data
...
{
    $sql = "SELECT username FROM employees WHERE username = ?"; //set up sql statement
...
function code_request($conn,$employ_id){
    ...
    $sql = "INSERT INTO temp (employ_id,code,time) VALUES(?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(1, $employ_id);
...
function activate_employ($conn,$employ_id)
{
    $sql = "UPDATE employees SET active = 1 WHERE employ_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(1,$employ_id);
    $stmt->execute();
    $conn = null;
