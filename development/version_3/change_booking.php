    ...
    try {
     ...
     the string before beginning the transfer to a time */
        ...
        if (book_update(dbconnect_insert(), $_SESSION['book_id'], $epoch_time)) {
            ...
            book_audit(dbconnect_insert(), $_SESSION['userid'], "log", "User has changed a booking");
...
foreach ($staff as $staf) {//dropdown menu
    ...
    if ($booking ['enginid'] == $staf ['enginid']) {
        $selected = "selected";
    } else {
        $selected = "";
    }
    echo "<option value =" . $staf ['enginid'] . " " . $selected . ">" . $state . " " . $staf['fname'] . " " .
        $staf['lname'] . " " . "</option>";
}
